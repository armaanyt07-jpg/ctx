...
from contextdrop.constants import BRAIN_FILE, CTX_DIR, HANDOFF_FILE, SYSTEM_PROMPT_FILE, TASKS_FILE, VERSION
...
def run() -> None:
    ...
    if not CTX_DIR.exists():
        ...
        for path in [BRAIN_FILE, TASKS_FILE, HANDOFF_FILE, SYSTEM_PROMPT_FILE]:
            if path.exists():
                ok(f"{path} exists")
            else:
